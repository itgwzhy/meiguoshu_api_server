...
func GetResponseMessage(code int) (message string) {
	...
	case PAY_REQUEST_POST_ERR:
		message = "支付请求失败" //
	case GACHAIN_TABLE_NOT_EXSIT:
		message = "Gachain表不存在"

	case GACHAIN_GET_DATA_FAILED:
		message = "gachain 获取数据失败"
	case GACHAIN_TABLE_NOT_RIGTH:
		message = "gachain数据表不对"
